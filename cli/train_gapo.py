...
from openrlhf.trainer import PPOTrainer, CLSTrainer
from openrlhf.utils import blending_datasets, get_strategy, get_tokenizer, get_cls_dataset,get_generator_output, mix_cls_dataset


def train(args):
    generator_training_turn = args.generator_training_turn
    ...
    dataset_cls_train_data, dataset_cls_test_data = get_cls_dataset(args.prompt_data)

    cls_warmup_train_data = random.sample(dataset_cls_train_data, k=args.cls_warmup_train_size)

    # 测试集保持不变
    cls_warmup_test_data = dataset_cls_test_data
    
    # warmup
    cls_trainer = CLSTrainer(reward_model,
                        cls_warmup_train_data,
                        cls_warmup_test_data,
                        args.cls_batch_size,
                        args.cls_learning_rate,
                        args.cls_warmup_epoch)
    cls_trainer.fit()
    # TODO: 保存 reward_model

    for epoch_index in range(adversarial_training_epoch):
        used_dataset_cls_train_data = dataset_cls_train_data[epoch_index * args.cls_train_size:(1+epoch_index) * args.cls_train_size]
        generative_cls_train_data, generative_cls_test_data = get_generator_output(data_dir=args.prompt_data, 
                                                                                    separator='# 生成内容',
                                                                                    max_train_samples=len(cls_train_data)*float(args.cls_generator_ratio),
                                                                                    max_test_samples=1000000000000000,
                                                                                    model=actor,
                                                                                    batch_size=2,
                                                                                    max_length=2048)
        cls_train_data = mix_cls_dataset(dataset_cls_train_data,
                                        generative_cls_train_data, 
                                        ratio=args.cls_generator_ratio)
        cls_test_data = dataset_cls_test_data + generative_cls_test_data

        print(f'Epoch：{epoch}，训练集总数：{len(cls_train_data)}，测试集总数：{len(cls_test_data)}')
        cls_trainer = CLSTrainer(reward_model,
                                cls_train_data,
                                cls_test_data,
                                args.cls_batch_size,
                                ...
                                args.cls_num_epoch)
        # TODO: 保存 reward_model
        # TODO: save_path得好好搞搞，以及read_path
...
if __name__ == "__main__":
    ...
    # Adversarial Process
    parser.add_argument("--cls_warmup_train_size", type=int, default=200)
    parser.add_argument("--cls_warmup_epoch", type=int, default=2)

    parser.add_argument("--cls_train_size", type=int, default=200)
    parser.add_argument("--cls_batch_size", type=int, default=4)
    parser.add_argument("--cls_learning_rate", type=float, default=1e-5)
    parser.add_argument("--cls_num_epoch", type=int, default=2)
    parser.add_argument("--cls_generator_ratio", type=int, default=0.5)
